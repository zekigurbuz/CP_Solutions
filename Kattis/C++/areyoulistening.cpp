...
signed main() {
	...
	double x, y; int n; cin >> x >> y >> n;
	double a[n][3]; forn(i, n) forn(j, 3) cin >> a[i][j];
	double d[n]; forn(i, n) d[i] = dist(x, y, a[i][0], a[i][1]);
	map<int, int> m; forn(i, n) m[max(0ll, (int) (ceil(d[i]) - a[i][2]))]++;
	int sum = 0;
	for(int i = 0;; i++) {
		sum += m[i];
		if (sum >= 3) {cout << i - 1 << endl; break; }
	}
	return 0;
}
