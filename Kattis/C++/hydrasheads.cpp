...
signed main() {
	...
	for(;;) {
		int a, b; cin >> a >> b; if (!a && !b) break;
		bool ret[200][200]; forn(i, 200) forn(j, 200) ret[i][j] = false;
		int ans = imax;
		queue<int> q; q.push(a); q.push(b); q.push(0);
		while (!q.empty()) {
			int h = q.front(); q.pop();
			int t = q.front(); q.pop();
			int s = q.front(); q.pop();
			...
			if (h >= 200 || t >= 200) continue;
			if (ret[h][t]) continue;
			ret[h][t] = true;
			if (t>0) {q.push(h); q.push(t+1); q.push(s+1);}
			if (t>1) {q.push(h+1); q.push(t-2); q.push(s+1);}
			if (h>1) {q.push(h-2); q.push(t); q.push(s+1);}
		}
