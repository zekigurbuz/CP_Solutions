...
signed main() {
	...
	while (n--) {
		...
		while (s.length() > 2) {
			cout << s << endl;
			int lastdigit = stoi(s.substr(s.length()-1,1));
			s = s.substr(0,s.length()-1);
			int lasttwodigits = stoi(s.substr(s.length()-2,2));
			lasttwodigits -= lastdigit;
			s = s.substr(0, s.length()-2) + to_string(lasttwodigits);
		}
		cout << s << endl;
		cout << "The number " << s << " is " << (stoi(s)%11==0?"":"not ") << "divisible by 11." << endl;
	}
	return 0;
}
