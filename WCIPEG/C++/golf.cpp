...
signed main() {
	...
	int d, n; cin >> d >> n; int a[n]; forn(i, n) cin >> a[i];
	int dp[d + 1]; forn(i, d + 1) dp[i] = -1;
	dp[0] = 0;
	for (int i = 1; i <= d; i++) {
		int mmin = imax;
		for (int j = 0; j < n; j++) {
			if (i >= a[j]) {
				if (dp[i-a[j]] != -1) {
					mmin = min(mmin, dp[i-a[j]]);
				}
			}
		}
		if (mmin != imax) dp[i] = mmin + 1;
	}
	if (dp[d] == -1) cout << "Roberta acknowledges defeat." << endl;
