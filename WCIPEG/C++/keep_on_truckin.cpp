...
signed main() {
	...
	int a, b, n; cin >> a >> b >> n;
	vi v;
	v.pb(0);v.pb(990);v.pb(1010);
	v.pb(1970);v.pb(2030);v.pb(2940);
	v.pb(3060);v.pb(4060);v.pb(4970);
	v.pb(5030);v.pb(5990);v.pb(6010);
	v.pb(7000);v.pb(3930);forn(i,n) {int x; cin >> x; v.pb(x);}
	n += 14; sort(v.begin(), v.end());
	int dp[7001]; forn(i, 7001) dp[i] = 0; dp[0] = 1;
	forn(i, n) {
		for(int j = i - 1; j >= 0; j--) {
			if (v[i]-v[j]>=a&&v[i]-v[j]<=b) dp[v[i]] += dp[v[j]];
		}
	}
	cout << dp[7000] << endl;
	return 0;
