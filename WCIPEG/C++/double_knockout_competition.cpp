...
signed main() {
	...
	int t; cin >> t; bool p = true;
	while (t--) {
		if (p) p = false;
		else cout << endl;
		int a, b=0, c=0; cin >> a;
		int q = 0;
		cout << "Round 0: " << a << " undefeated, 0 one-loss, 0 eliminated" << endl;
		while (a+b>1) {
			q++;
			if (a==1&&b==1) {
				b=2;
			...
			cout << "Round " << q << ": " << a << " undefeated, " << b << " one-loss, " << c << " eliminated" << endl;
		}
		cout << "There are " << q << " rounds." << endl;
	}
	return 0;
}
