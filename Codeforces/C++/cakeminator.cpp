...
signed main() {
	...
	int r, c; cin >> r >> c;
	char a[r][c]; forn(i, r) forn(j, c) cin >> a[i][j];
	bool b[r][c]; forn(i, r) forn(j, c) b[i][j] = false;
	forn(i, r) {
		bool d = true;
		forn(j, c) if (a[i][j] == 'S') d = false;
		if(d) forn(j, c) b[i][j] = true;
	}
	forn(j, c) {
		bool d = true;
		forn(i, r) if (a[i][j] == 'S') d = false;
		if(d) forn(i, r) b[i][j] = true;
	}
	int e = 0;
	forn(i, r) forn(j, c) if (b[i][j]) e++;
	cout << e << endl;
	return 0;
}
