...
signed main() {
	...
	int n; cin >> n; pii a[n]; forn(i, n) {int x; cin >> x; a[i] = mp(x, i);}
	sort(a, a+n); reverse(a, a+n); int b[n]; int t = 1, aa=0;
	forn(i, n) {
		if (i && a[i-1].fi > a[i].fi) {t++;t+=aa;aa=0;}
		else if (i && a[i-1].fi == a[i].fi) aa++;
		b[a[i].se] = t;
	}
	forn(i, n) {
		if (i) cout << " ";
