...
signed main() {
	...
	int n; cin >> n; int a[3]; forn(i, 3) a[i] = 0;
	string s[3]; s[0] = "chest"; s[1] = "biceps"; s[2] = "back";
	forn(i, n) {
		int x; cin >> x;
		a[i%3] += x;
	}
	if (max(a[0], max(a[1], a[2])) == a[0]) cout << s[0] << endl;
	else if (max(a[0], max(a[1], a[2])) == a[1]) cout << s[1] << endl;
	else cout << s[2] << endl;
	return 0;
}
