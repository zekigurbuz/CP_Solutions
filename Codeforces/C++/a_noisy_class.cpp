...
signed main() {
	...
	int n, m; cin >> n >> m; vi vv;
	vector<vi> v; forn(i, n) v.pb(vv);
	forn(i, m) {int a, b; cin >> a >> b; v[--a].pb(--b);}
	bool noLoop = true;
	forn(i, n) {
		if (v[i].empty()) continue;
		qi q; q.push(i);
		bool b[n]; memset(b, false, n);
		while (!q.empty()) {
			int cur = q.front(); q.pop();
			if (cur == i && b[i]) { noLoop = false; break;}
			if (b[cur]) continue;
			b[cur] = true;
