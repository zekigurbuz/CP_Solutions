...
int dist(char a, char b) {return min(max(a,b)-min(a,b), 'Z'-max(a,b)+min(a,b)-'A' + 1);}
signed main() {
	ios_base::sync_with_stdio(0);cin.tie(0);
	int n; cin >> n; char c[n]; forn(i, n) cin >> c[i];
	int ans = imax;
	forn(i, n - 3) {
		ans = min(ans, dist('A', c[i]) + dist('C', c[i+1]) + dist('T', c[i+2]) + dist('G', c[i+3]));
	}
	cout << ans << endl;
	return 0;
}
