...
signed main() {
	...
	int n, m; cin >> n >> m;
	ll a[(int)2e5]; forn(i, 2e5) a[i] = 200000;
	queue<int> q; q.push(n); q.push(0);
	while (!q.empty()) {
		int steps = q.front(); q.pop();
		int val = q.front(); q.pop();
		if (a[val] < steps) continue;
		if (val > 2 * m || val < 0) break;
		a[val] = steps;
		q.push(val << 1); q.push(++steps);
		q.push(--val); q.push(steps);
	}
	cout << a[m] << endl;
	return 0;
}
